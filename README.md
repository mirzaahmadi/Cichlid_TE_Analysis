...
## Bare-minimum files needed to run (after cloning)

Cloning the repo gives you the code and docs. To actually run the pipeline you
must add the **input data files below yourself** — none of them are generated by
any script, and they are too large to keep in the repo. 

Files you add are marked `← ADD`. Place them exactly as shown:

```
cichlid-TE-pipeline/
├── README.md                                     (in repo)
├── FUTURE_DIRECTIONS.docx                         (in repo)
├── .gitignore                                     (in repo)
├── Orthogroups.tsv                            ← ADD  (OrthoFinder output)
├── Orthogroups_SingleCopyOrthologues.txt      ← ADD  (OrthoFinder output)
├── scripts/                                        (in repo)
│   ├── 1_extract_orthogroup_gene_info.py
│   ├── 2_te_flanking_analysis.R
│   ├── 3_cross_species_aggregation.R
│   └── 4_top10_ortholog_neighborhoods.R
├── docs/                                           (in repo)
│   ├── FULL_PIPELINE_WORKFLOW.docx
│   └── PIPELINE_SCRIPT_MAP.html
├── gff3/                                       ← ADD  (used by Stage 1)
│   ├── Metriaclima_zebra_AF.annotation.gff3
│   ├── Pundamilia_nyererei_AF.annotation.gff3
│   ├── Oreochromis_niloticus_AF.annotation.gff3
│   └── Neolamprologus_brichardi_AF.annotation.gff3
└── data/                                       ← ADD
    ├── GENE_Metriaclima_zebra_AF.annotation.gff3       (used by Stage 2)
    ├── GENE_Pundamilia_nyererei_AF.annotation.gff3
    ├── GENE_Oreochromis_niloticus_AF.annotation.gff3
    ├── GENE_Neolamprologus_brichardi_AF.annotation.gff3
    ├── TE_Metriaclima_zebra_AF_genome.fa.out          (RepeatMasker .out)
    ├── TE_Pundamilia_nyererei_AF_genome.fa.out
    ├── TE_Oreochromis_niloticus_AF_genome.fa.out
    └── TE_Neolamprologus_brichardi_AF_genome.fa.out

```
## Where to get the data

Two download bundles:

- **Data to run the current pipeline** — exactly the files listed above and
  nothing more; enough to reproduce the four-African-cichlid run end to end.
  [Download link](https://drive.google.com/drive/folders/1gfNqExegLm2k5K9BsHDKpCTjirrWZTiS?usp=sharing)
- **Full data set (for later / extended work)** — all data collected for the
  project, including the South American cichlid genomes and TE annotations for
  future extensions beyond the current pipeline.
  [Download link](https://drive.google.com/file/d/1afkSDFI8llA4Ie1KAeKigkXk0Mdw7sOp/view?usp=sharing)
